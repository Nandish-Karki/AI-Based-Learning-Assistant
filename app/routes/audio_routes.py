...
def generate_module_audio():
    try:
        ...
        data = request.json
        # Use user_email from token for consistency
        ...
        modules = get_cached_modules(user_email, document_id) # Use user_email from token
        if not modules:
        ...
        public_url = save_module_audio(user_email, document_id, module_number, ssml, audio_path, speech_marks) # Use user_email from token
